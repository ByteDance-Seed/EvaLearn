...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("demonstration_learning_eval.log"),
...
class GPTAnnotator:
    ...
    def inference_with_demonstration(self, system_prompt, demo_prompt, prompt_list):
        """Generate model inference with demonstration learning context."""
        prompt = ""
        ...
        final_prompt = demo_prompt + "**All demonstrations end, now please start answering the final question.**\n" + '\n[Question]:\n' + prompt
...
def get_demonstration_prompt(i, item):
    """Generate a formatted demonstration prompt from an item (with all previous solutions)."""
    return f"[Demonstration Record {i+1} Start]\n[Question Start]\n{item['prompt']}\n[Question End]\n[Standard Answer Start]\n{item['rubric_zh']}\n[Standard Answer End]\n[Demonstration Record {i+1} End]\n\n"

def demonstration_infer_and_judge(annotator, data_idx, system_prompt, seq, seq_id):
    """Process a sequence of questions with demonstration learning."""
    demonstrations = []
    results = []
    ...
    for i in tqdm(range(len(questions)), desc=f"Processing sequence {seq_id}"):
        ...
        demo_prompt = ''.join(demonstrations)
        prompt_list = item['prompt']
        rubric_zh = item["rubric_zh"]

        start_time = time.time()
        
        # Inference with demonstration context
        model_output = annotator.inference_with_demonstration(system_prompt, demo_prompt, prompt_list)
        ...
        demonstrations.append(get_demonstration_prompt(i, item))
        results.append(item)
...
def process_sequence_batch(batch_seqs, data_idx, system_prompts, annotator, all_results, output_json_path, results_lock):
    ...
    for seq in batch_seqs:
        ...
        seq_results = demonstration_infer_and_judge(annotator, data_idx, system_prompt, seq, seq_id)
...
def demonstrationEval(input_json_path, seq_json_path, output_json_path, worker_nums=None, 
                      check_empty=True, judge_api_key=None, client_api_key=None, 
                      judge_model="gpt-4o-2024-11-20", client_model="gpt-4o-2024-11-20",
                      judge_api_base_url=None, client_api_base_url=None):
    """Main function for demonstration learning evaluation of model performance."""
    worker_nums = worker_nums or CONFIG["worker_nums"]
    start_time = time.time()
    
    logger.info(f"Starting demonstration learning evaluation using {worker_nums} worker threads")
    ...
    system_prompts = {
        item['type']: f"You are a student. You need to complete a question on [{item['type']}] skills. Before the final question is presented, I will share with you example solutions from previous questions. These demonstrations show correct solutions and their standard answers. You can learn from these demonstrations to better understand the task, improve your [{item['type']}] skills, and perform better on the final question.\n\nWhen you see the final question, please apply the patterns and techniques you've learned from the demonstrations to provide the best possible answer.\n\nRemember:\n1. Carefully understand the requirements of each question by studying the demonstrations.\n2. Identify the patterns and approaches used in the standard answers.\n3. Apply these techniques to solve the final question correctly.\n4. Ensure your answer fully meets all requirements.\n5. You may reference specific examples from the demonstrations if relevant.\n\nNow, let's begin this demonstration learning process."
        for item in data
    }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Demonstration Learning Evaluation Tool")
