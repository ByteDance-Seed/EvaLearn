...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("few_shot_eval.log"),
...
def select_few_shot_examples(data, seq_datas):
    """Select few-shot examples for each task type."""
    ...
    logger.info(f"Data grouped by type: {len(data_by_type)} types found")
    for item_type, items in data_by_type.items():
        logger.info(f"  - {item_type}: {len(items)} items")
    
    # Select few-shot examples for each type
    few_shot_examples = {}
    few_shot_ids = set()
    
    for item_type, items in data_by_type.items():
        if len(items) >= CONFIG["few_shot_examples_per_type"]:
            selected = random.sample(items, CONFIG["few_shot_examples_per_type"])
            few_shot_examples[item_type] = selected
            
            # Record selected IDs
            selected_ids = [item.get('id') for item in selected]
            few_shot_ids.update(selected_ids)
            logger.info(f"Selected few-shot examples for {item_type}: {selected_ids}")
    
    logger.info(f"Total few-shot example IDs selected: {len(few_shot_ids)}")
    
    return few_shot_examples, few_shot_ids
...
class GPTAnnotator:
    ...
    def inference_with_few_shot(self, system_prompt, few_shot_prompt_list):
        """Generate model inference with few-shot examples (no feedback)."""
        # Build few-shot context
        prompt = ""
        
        # Add few-shot examples (questions + canonical answers, no feedback)
        for i in range(0, len(few_shot_prompt_list) - 1, 2):
            if i + 1 < len(few_shot_prompt_list):
                prompt += f"[History Record {i//2 + 1} Start]\n"
                prompt += f"[Question Start]\n{few_shot_prompt_list[i]}\n[Question End]\n"
                prompt += f"[Standard Answer Start]\n{few_shot_prompt_list[i+1]}\n[Standard Answer End]\n"
                prompt += f"[History Record {i//2 + 1} End]\n\n"
        
        # Add final question
        prompt += "**All few-shot examples end, now please start answering the final question.**\n\n"
        prompt += f"[Question]:\n{few_shot_prompt_list[-1]}"
...
def few_shot_infer_and_judge(annotator, data_idx, system_prompt, seq, seq_id, few_shot_examples):
    """Process a sequence of questions with few-shot examples."""
    results = []
    questions = seq['question_ids']
    seq_type = seq.get('type', 'unknown')
    
    # Get few-shot examples for this sequence type
    few_shot_examples_for_type = few_shot_examples.get(seq_type, [])
    
    # Build few-shot prompt list (question + canonical answer pairs)
    few_shot_prompt_list = []
    for example in few_shot_examples_for_type:
        # Add question (last prompt item)
        if isinstance(example.get('prompt'), list):
            few_shot_prompt_list.append(example['prompt'][-1])
        else:
            few_shot_prompt_list.append(example.get('prompt', ''))
        # Add canonical answer
        few_shot_prompt_list.append(example.get('canonical_answer', ''))
    ...
    for i in tqdm(range(len(questions)), desc=f"Processing sequence {seq_id}"):
        ...
        # Build few-shot prompt for this item
        current_few_shot_prompt = few_shot_prompt_list + [
            prompt_list[-1] if isinstance(prompt_list, list) else prompt_list
        ]
        
        # Inference and judging
        model_output = annotator.inference_with_few_shot(system_prompt, current_few_shot_prompt)
...
def process_sequence_batch(batch_seqs, data_idx, system_prompts, annotator, all_results, output_json_path, results_lock, few_shot_examples):
    ...
    for seq in batch_seqs:
        ...
        seq_results = few_shot_infer_and_judge(annotator, data_idx, system_prompt, seq, seq_id, few_shot_examples)
...
def fewShotEval(input_json_path, seq_json_path, output_json_path, worker_nums=None, 
               check_empty=True, judge_api_key=None, client_api_key=None, 
               judge_model="gpt-4o-2024-11-20", client_model="gpt-4o-2024-11-20",
               judge_api_base_url=None, client_api_base_url=None):
    """Main function for few-shot evaluation of model performance."""
    worker_nums = worker_nums or CONFIG["worker_nums"]
    start_time = time.time()
    
    logger.info(f"Starting few-shot evaluation using {worker_nums} worker threads")
    ...
    few_shot_examples, few_shot_ids = select_few_shot_examples(data, seq_datas)
    
    # Create indices
    data_idx = {item['id']: item for item in data}
    system_prompts = {
        item['type']: f"You are a student. You need to complete a question on [{item['type']}] skills. Before the final question is presented, I will show you some example questions with their standard answers. You can learn from these examples to better understand the task requirements and perform well on the final question.\n\nRemember:\n1. Carefully understand the requirements of each question.\n2. Ensure your answer fully meets all requirements.\n3. Learn from the provided examples to understand what a good answer looks like.\n\nNow, let's begin."
        for item in data
    }
    ...
    if worker_nums > 1 and len(remaining_seqs) > 1:
        ...
        with ThreadPoolExecutor(max_workers=worker_nums) as executor:
            futures = [
                executor.submit(process_sequence_batch, batch, data_idx, system_prompts, 
                              annotator, all_results, output_json_path, results_lock, few_shot_examples)
        ...
        process_sequence_batch(remaining_seqs, data_idx, system_prompts, annotator, 
                             all_results, output_json_path, results_lock, few_shot_examples)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Few-Shot Evaluation Tool")
